...
export default function CorbuBeachInfo() {
  const t = useTranslations('corbuKnowledge');
  const messages = useMessages() as any;
  const sections = (messages?.corbuKnowledge?.sections || []) as Array<{ id: string; title: string; content: string }>;
          ...
          {sections.map((section, index) => (
            ...
            >
              <div className="flex-1 w-full bg-white/5 p-8 rounded-2xl border border-white/10 shadow-sm hover:shadow-md transition-shadow">
                ...
                <p
                  className="text-lg leading-relaxed ml-14 whitespace-pre-wrap"
